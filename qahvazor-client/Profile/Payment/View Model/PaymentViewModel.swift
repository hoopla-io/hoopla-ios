...
final class PaymentViewModel {
    ...
    func getListPayment() {
        ...
        Task { [weak self] in
            await JSONDownloader.shared.jsonTask(url: EndPoints.payServices.rawValue, requestMethod: .get, completionHandler: { [weak self]  (result) in
    ...
    func topUp(serviceId: Int, amount: String) {
        ...
        Task { [weak self] in
            await JSONDownloader.shared.jsonTask(url: EndPoints.topUp.rawValue, requestMethod: .get, parameters: params, completionHandler: { [weak self]  (result) in
                guard let self = self else { return }
                switch result {
                case .Error(let error, let message):
                    self.delegate?.showAlertClosure(error: (error,message))
